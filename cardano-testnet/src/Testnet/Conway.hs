...
import           Testnet.Options
                   (BabbageTestnetOptions (babbageEpochLength, babbageNodeLoggingFormat, babbageNumSpoNodes, babbageProtocolVersion, babbageSecurityParam),
...
conwayTestnet :: BabbageTestnetOptions -> H.Conf -> H.Integration TestnetRuntime
conwayTestnet testnetOptions H.Conf {..} = do
  ...
  H.rewriteJsonFile (genesisShelleyDir </> "genesis.json") $ J.rewriteObject
    ...
    . HM.insert "maxLovelaceSupply"      (toJSON @Int 1_000_000_000_000)
    . HM.insert "minFeeA"                (toJSON @Int 44)
    . HM.insert "minFeeB"                (toJSON @Int 155_381)
    . HM.insert "minUTxOValue"           (toJSON @Int 1_000_000)
    . HM.insert "decentralisationParam"  (toJSON @Double 0.7)
    . flip HM.adjust "protocolParams"
      ( J.rewriteObject
        ( flip HM.adjust "protocolVersion"
          ( J.rewriteObject ( HM.insert "major" (toJSON @Int $ babbageProtocolVersion testnetOptions))) -- not needed for Conway (yet)
        )
      )
